...
> **Known stack wrinkle (surface to the user during the `result` layer):** ADR 0005 says "Kotlin's `Result<T>` (stdlib)", but every kmp-forge signature uses the two-param `Result<T, DomainError>` with `onFailure { error: DomainError }`, which stdlib `Result<T>` cannot express. The migrator uses an existing project two-param Result if present, else establishes a project-local one and reports it. Flag this so the user decides (project-local type vs a lib like kotlin-result) — and consider fixing ADR 0005's wording in the plugin.
